...
public class normap extends Frame implements Runnable{
	...
	public void paint(Graphics g){
		...
		if(flag)
			...
			for(p=111d; p>=0.15d; p-=p<1?0.01d:p<2?0.025d:p<10?0.1d:p<20?0.25d:p<40?0.5d:p<80?5d:10d)
				desen(g, p);
		flag = !flag;
	}
	...
	void patrat1(Graphics g){
		int[] x = {0, w/2, w, w/2}, y = {h/2, 0, h/2, h};
		patrat(g, x, y, true);
	}
	...
	double f(double x, double p){
		return Math.pow((Math.pow(200, p) - Math.pow(Math.abs(x), p)), 1/p);
	}
